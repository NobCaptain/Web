...
- CSS使用 `border-top-left-radius`、`border-top-right-radius`、`border-bottom-left-radius`、`border-bottom-right-radius` 属性来设置圆角边框的左上角、右上角、左下角、右下角。
...
- `border-top-left-radius`、`border-top-right-radius`、`border-bottom-left-radius`、`border-bottom-right-radius` 属性的属性值分别取值为：

  - `border-top-left-radius` 属性：`数字+%`、像素值（设置左上角的圆的半径为多少像素）。

    > 注：%是一个相对单位，如：两个嵌套关系（父子关系）的块级元素，子元素的宽度设置为 `width: 50%`，则表示设置子元素的宽度为父元素的宽度的50%，这里表示的是将圆的半径设置为父元素的宽度的百分之几。

  - `border-top-left-radius` 属性：`数字+%`、像素值（设置右上角的圆的半径为多少像素）。

  - `border-bottom-left-radius` 属性：`数字+%`、像素值（设置左下角的圆的半径为多少像素）。

  - `border-bottom-right-radius` 属性：`数字+%`、像素值（设置右下角的圆的半径为多少像素）。
