...
- Axios的使用步骤：
  ...
  2. 使用Axios发起 `get` 请求：
     ...
     - 注意点：
       ...
       - 当使用 `get()` 方法发起 `get` 请求，并且请求成功后，`get()` 方法会将从服务器上获取的数据，进行包装，然后将包装好的数据传递给 `then()` 方法中的回调函数的第一个形参中，所以回调函数的括号内的第一个形参中存储的就是包装好的数据，又因为包装好的数据是以对象的形式来存储的，所以回调函数的括号内的第一个形参的名字就是该对象的对象名，一般我们会将回调函数的括号内的第一个形参的名字设置为 `res`，所以当我们要获取服务器响应回来的数据时，就要通过该对象的 `data` 属性来获取，也就是使用 `res.data` 来获取。
  ...
  3. 使用Axios发起 `post` 请求：
     ...
     - 注意点：
       ...
       - 当使用 `post()` 方法发起 `post` 请求，并且请求成功后，`post()` 方法会将请求成功的信息，进行包装，然后将包装好的信息传递给 `then()` 方法中的回调函数的第一个形参中，所以回调函数的括号内的第一个形参中存储的就是包装好的信息，又因为包装好的信息是以对象的形式来存储的，所以回调函数的括号内的第一个形参的名字就是该对象的对象名，一般我们会将回调函数的括号内的第一个形参的名字设置为 `res`，所以当我们要获取服务器响应回来的数据时，就要通过该对象的 `data` 属性来获取，也就是使用 `res.data` 来获取。
  ...
  4. 使用Axios发起 `get` 请求或 `post` 请求：
     ...
     - 注意点：
       ...
       - 当使用 `axios()` 方法发起 `get` 请求，并且请求成功后，`axios()` 方法会将从服务器上获取的数据，进行包装，然后将包装好的数据传递给 `then()` 方法中的回调函数的第一个形参中，所以回调函数的括号内的第一个形参中存储的就是包装好的数据，又因为包装好的数据是以对象的形式来存储的，所以回调函数的括号内的第一个形参的名字就是该对象的对象名，一般我们会将回调函数的括号内的第一个形参的名字设置为 `res`，所以当我们要获取服务器响应回来的数据时，就要通过该对象的 `data` 属性来获取，也就是使用 `res.data` 来获取。

       - 当使用 `axios()` 方法发起 `post` 请求，并且请求成功后，`axios()` 方法会将请求成功的信息，进行包装，然后将包装好的信息传递给 `then()` 方法中的回调函数的第一个形参中，所以回调函数的括号内的第一个形参中存储的就是包装好的信息，又因为包装好的信息是以对象的形式来存储的，所以回调函数的括号内的第一个形参的名字就是该对象的对象名，一般我们会将回调函数的括号内的第一个形参的名字设置为 `res`，所以当我们要获取服务器响应回来的数据时，就要通过该对象的 `data` 属性来获取，也就是使用 `res.data` 来获取。
