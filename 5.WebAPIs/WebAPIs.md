...
- 注意点：因为 `window` 对象是JavaScript中的顶级对象，所以 `history` 对象中的属性和方法，在调用的时候可以省略 `window`。
...
- 正则表达式中的字符类是用来匹配字符集合的，字符类主要有以下两个字符：
  ...
  | 字符类 | 作用                                                         |
  | :----: | ------------------------------------------------------------ |
  |  `[]`  | 匹配字符集合，如：正则表达式 `/[abc]/.test('a')` 表示被检测的字符串中只要包含了小写的 `a`、`b`、`c` 三个英文字母中的任意一个字符，就返回 `true` 否则就返回 `false` |
